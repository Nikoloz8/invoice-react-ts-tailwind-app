...
export default function ConfirmDeletion({ confirmDeletion, setConfirmDeletion }: TConfirmDeletion) {
    ...
    return (
        <div className={`${!confirmDeletion ? "bg-[rgba(0,0,0,0)] z-[-1]" : undefined} fixed top transition-bg duration-1000 left-0 top-0 w-full h-full bg-[rgba(0,0,0,0.2)] flex items-center justify-center`}>
            <div className={`ml-[104px] max-lg:ml-0 max-sm:w-[327px] max-sm:p-[24px] bg-[#FFFFFF] rounded-[8px] p-[48px] max-w-[490px] flex flex-col gap-[20px] ${toggle ? "bg-[#1E2139]!" : undefined}`}>
                <h2 className={`${H2} ${toggle ? "text-[#FFFFFF]!" : undefined}`}>Confirm Deletion</h2>
                <p className={`${P1} text-[#888EB0] ${toggle ? "text-[#DFE3FA]" : undefined}`}>Are you sure you want to delete invoice #{invoice?.id}? This action cannot be undone.</p>
