    ...
    return (
        <div className="flex flex-col gap-[16px] m-[50px_0_50px_0]">
            {filterInvoices(invoices, filters).length > 0 ? filterInvoices(invoices, filters).map((e, index) => {
                return <div key={index} onClick={() => navigate(`/details/${e.id}`)} className="w-[100%] p-[0_24px_0_32px] items-center h-[72px] bg-[#FFFFFF] flex justify-between cursor-pointer rounded-[8px]">
                    ...
                    <div className="flex gap-[32px] items-center">
                        ...
                        <div className="flex items-center gap-[16px]">
                            <div className={`${H4} ${e.status.toLowerCase() === "paid" ? "text-[#33D69F]! bg-[rgba(51,214,159,0.1)]" : e.status.toLowerCase() === "pending" ? "text-[#FF8F00]! bg-[rgba(255,143,0,0.1)]" : "text-[#373B53]! bg-[rgba(55,59,83,0.1)]"}  rounded-[6px] w-[104px] h-[40px] flex items-center justify-center gap-[8px]`}> <span className={`w-[8px] h-[8px] mb-[3px]! ${e.status.toLowerCase() === "paid" ? "bg-[#33D69F]" : e.status.toLowerCase() === "pending" ? "bg-[#FF8F00]" : "bg-[#373B53]"} rounded-full`}></span>{e.status[0].toUpperCase()}{e.status.slice(1)}</div>
                ...
                <div className="w-[207px]  flex flex-col gap-[20px]">
                    <h2 className={`${H2} text-center`}>There is nothing here</h2>
                    <p className={`${P2} text-[#888EB0] text-center`}>
